...
| F-SCALE-141 | Licentieserver-SPOF | **CODE DICHT** (F-SCALE-141b client-helft + **F-SCALE-141c server-helft**: `configEndpoint_` emit nu de standby-URL uit `STANDBY_SERVER_URL`; 141b was zonder die server-wiring inert — nu end-to-end geborgd). Runbook klaar (`standby-server-runbook.md`). | **Sam**: tweede deploy + `STANDBY_SERVER_URL` zetten + uptime-monitor op `?actie=health` (4 afvink-criteria in de runbook). |
