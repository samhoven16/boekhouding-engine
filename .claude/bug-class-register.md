...
|---|--------|----------------|-----------------------------------|----------|--------|
| 1 | **Verkeerde sheet-kolom-index** | bare `data[i][N]` magische nummers; semantiek niet in de naam | EUVerkoop `[21]` vs `[7]` (F-TAX-130) · Mollie `[0]/[6]` vs `[1]/[12]` (F-PAY-130) · XAF accTp (F-ACC-009) | **Gebouwd:** `KOL`-accessor (`src/SheetKolom.gs`) als enige bron + contract-test `contract-sheet-kolom.test.js` (KOL ⇄ `sheet-schemas.md`, geen drift) + ban-test (gemigreerd bestand mag geen bare `data[i][N]`). **Gemigreerd:** EUVerkoop + Mollie (de 2 bewezen-buggy bestanden). **Resteert:** de overige sheet-lezende bestanden migreren (BTW, Boekingen, Verkoop-/Inkoopfacturen, XafExport(40), Triggers, Dashboard, Rapportages, Bankimport) — incrementeel, elk + ban-uitbreiding, met de testsuite als vangnet. | 🟠 PARTIAL — mechanisme dicht, migratie loopt |
...
| 9 | **Geld-precisie / float-drift / afronding** | euro's-as-float i.p.v. integer-cent; `Math.round(n*100)/100` met float-fout; asymmetrische afronding bij negatief; inconsistent afrond-moment | `rondBedrag_(1.005)=1.00` (P0) · €0,01-divergentie factuur↔grootboek (te verifiëren) | **Nodig:** geld in **integer-centen** + één deterministisch afrondpunt; `rondBedrag_` exact maken (epsilon/string-exponent) + property-test (Σ-deel = geheel op de cent; round-trip stabiel; symmetrisch bij negatief). Volgt het **`calculation-register.md`** per berekening. | 🔴 OPEN — precisie-audit-ronde 2026-06-19 loopt |
