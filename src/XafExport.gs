...
function exporteerXaf() {
  ...
  try { ss.toast('XAF-export wordt voorbereid, even geduld…', 'Export', 5); } catch (_) {}
  ...
  const bestandsnaam = 'XAF_' + bedrijf + '_' + jaar + '_' + Utilities.formatDate(new Date(), 'Europe/Amsterdam', 'yyyy-MM-dd') + '.xaf.xml';
...
function _bouwTransactionsXml_(ss, jaar) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    xml += '          <trLine>\n';
    xml += '            <nr>1</nr>\n';
    xml += '            <accID>' + _xafEsc_(debet) + '</accID>\n';
    xml += '            <docRef>' + _xafEsc_(id) + '</docRef>\n';
    xml += '            <effectiveDate>' + datum + '</effectiveDate>\n';
    xml += '            <desc>' + _xafEsc_(omschr) + '</desc>\n';
    xml += '            <amnt>' + bedrag.toFixed(2) + '</amnt>\n';
    xml += '            <amntTp>D</amntTp>\n';
    xml += '          </trLine>\n';
    ...
    xml += '          <trLine>\n';
    xml += '            <nr>2</nr>\n';
    xml += '            <accID>' + _xafEsc_(credit) + '</accID>\n';
    xml += '            <docRef>' + _xafEsc_(id) + '</docRef>\n';
    xml += '            <effectiveDate>' + datum + '</effectiveDate>\n';
    xml += '            <desc>' + _xafEsc_(omschr) + '</desc>\n';
    xml += '            <amnt>' + bedrag.toFixed(2) + '</amnt>\n';
