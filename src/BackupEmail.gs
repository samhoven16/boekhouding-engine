  ...
  try {
    if (bytes <= MAX_INLINE_BYTES) {
      const body =
        ...
        '— Boekhoudbaar';
      MailApp.sendEmail({  // klant-mail-ok: opt-in wekelijkse backup ('Backup-email actief', default uit) - data-bescherming
        to: adres,
        subject: onderwerp,
        body: body,
      ...
      MailApp.sendEmail(adres, onderwerp + ' (download-link)', body);  // klant-mail-ok: backup-download-link (opt-in backup, data-bescherming)
    }
